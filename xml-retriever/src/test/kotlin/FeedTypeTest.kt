...
class FeedTypeTest {
    ...
    fun mimeTypeToFeedType() {
        assertEquals(
            FeedType.ATOM,
            FeedType.detectFeedType(readSampleFile("/atom-feed.xml"))
        )
        assertEquals(
            FeedType.RSS,
            FeedType.detectFeedType(readSampleFile("/baeldung.xml"))
        )
        assertEquals(
            FeedType.RSS,
            FeedType.detectFeedType(readSampleFile("/sample-rss-2.xml"))
        )

        assertThrows<UnknownFeedType> {
            FeedType.detectFeedType(
                "<root/>"
            )
        }
    }
