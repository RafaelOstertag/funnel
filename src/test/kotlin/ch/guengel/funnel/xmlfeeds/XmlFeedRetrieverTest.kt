...
class XmlFeedRetrieverTest {
    ...
    fun `atom happy path`() {
        ...
        val result = atomFeedRetriever.retrieve(
                ZonedDateTime.parse("2000-01-01T00:00:00Z"))

        assertEquals("urn:uuid:60a76c80-d399-11d9-b93C-0003939e0af6",
                result.id)
    ...
    fun `rss happy path`() {
        ...
        assertEquals("http://liftoff.msfc.nasa.gov/",
                result.id)
    ...
    fun `feed not found`() {
        ...
        val atomFeedRetriever = XmlFeedRetriever(httpTransport)
        assertThrows<HttpError> { atomFeedRetriever.retrieve(ZonedDateTime.parse("2001-01-01T00:00:00Z")) }
    ...
    fun `invalid content`() {
        ...
        val atomFeedRetriever = XmlFeedRetriever(httpTransport)
        assertThrows<CannotDeserializeXML> { atomFeedRetriever.retrieve(ZonedDateTime.parse("2001-01-01T00:00:00Z")) }
    }
    ...
    fun `invalid content type`() {
        ...
        val atomFeedRetriever = XmlFeedRetriever(httpTransport)
        assertThrows<UnknownFeedType> { atomFeedRetriever.retrieve(ZonedDateTime.parse("2001-01-01T00:00:00Z")) }
    }
